...
const ProcessesPage: React.FC = () => {
  ...
  const getStatusChip = (serverId: string) => {
    ...
    return (
      <Chip
        label={t(`process.status.${status.status}`)}
    ...
    <Box>
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>{t('process.fields.name')}</TableCell>
              <TableCell>{t('process.fields.command')}</TableCell>
              <TableCell>{t('process.fields.platform')}</TableCell>
              <TableCell>{t('process.fields.status')}</TableCell>
              <TableCell align="center">{t('process.fields.autoStart')}</TableCell>
              <TableCell align="right">{t('process.fields.actions')}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {servers.map((server) => {
                ...
                <TableRow key={server.id}>
                  ...
                  <TableCell align="right">
                    <Tooltip title={isRunning ? t('process.stop') : t('process.start')}>
                      <IconButton
                        size="small"
                        onClick={() => isRunning ? handleStop(server.id) : handleStart(server.id)}
                      >
